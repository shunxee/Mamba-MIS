...
class TransFuse_S(nn.Module):
    def __init__(self, num_classes=1, drop_rate=0.2, normal_init=True, pretrained=False, image_size=256):
        super(TransFuse_S, self).__init__()
        self.grid = (image_size // 16, image_size // 16)
        ...
        self.transformer = deit(pretrained=pretrained, img_size=image_size)
    ...
    def forward(self, imgs, labels=None):
        ...
        x_b = x_b.reshape(x_b.shape[0], -1, imgs.shape[-2] // 16, imgs.shape[-1] // 16)
        x_b = self.drop(x_b)
